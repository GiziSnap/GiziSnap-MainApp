...
export function PushNotificationManager() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      const { subscription } = JSON.parse(storedData) as {
      ...
      if (Notification.permission === 'granted') {
        ...
        toast.warning(
          'Push notifications permission was not granted. Subscription data has been removed.',
        );
      }
    }
  }, []);
  ...
  async function subscribeToPush() {
    try {
      ...
      if (
        ...
        error.message === 'Permission not granted for Notification'
      ) {
    ...
    if (subscription) {
      try {
        if (typeof subscription.unsubscribe === 'function') {
          ...
          console.warn(
            'Subscription object does not have an unsubscribe method.',
          );
    ...
    <div>
      {subscription ? (
        <>
          <Button
            variant={'outline'}
            className='cursor-pointer border-none p-0 hover:bg-transparent'
