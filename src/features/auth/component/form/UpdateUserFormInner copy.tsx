...
export const UpdateUserFormInner = ({
  ...
  return (
    <form id={formId} onSubmit={form.handleSubmit(onSubmit)}>
      {/* FormField untuk username, email, dan password */}
      <FormField
      ...
      {/* <FormItem className="mt-4">
        <FormLabel className="text-lg font-semibold text-gray-800">Foto Profil</FormLabel>
        <div className="flex items-center gap-4 mt-2">
          {previewImage && (
            <Image
              ...
              width={100}
              height={100}
              className="rounded-full border-2 border-green-300 shadow-md object-cover"
            />
          )}
          <div className="flex flex-col gap-2">
            <UploadButton
              className='flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-green-500 rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500'
              endpoint="imageUploader"
              onClientUploadComplete={(res) => {
                if (res && res.length > 0 && res[0]?.serverData) {
                  const url = (res[0].serverData as { uploadedBy: string; fileUrl: string; }).fileUrl;
